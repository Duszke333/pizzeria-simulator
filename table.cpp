...
Table::Table(unsigned int id, TableSize the_size, const Menu& the_menu) :
    ready(true),
    table_id(id),
    earned(0),
    size(the_size),
...
void Table::set_group(Group the_group) {
    if (the_group.get_group_size() > static_cast<unsigned int>(size)) throw InvalidGroupSizeException();
    group = the_group;
}
...
void Table::update_status() noexcept {
    switch (status) {
        ...
        case Status::WaitingForDrinks:
            if (order.ready_to_serve[0]) ready = true;
            break;
        case Status::WaitingForAppetizers:
            if (order.ready_to_serve[1]) ready = true;
            break;
        case Status::WaitingForPizzas:
            if (order.ready_to_serve[2]) ready = true;
            break;
        default:
            break;
    }
...
std::string Table::interact(unsigned int seed) {
    if (!ready) throw TableNotReadyException();
    std::string output = "Table no. " + std::to_string(table_id) + ": ";
    switch (status) {
        case Status::Free:
            output +=  "The group no. " + std::to_string(group.get_id()) +  " sat at the table and is waiting for menus.";
